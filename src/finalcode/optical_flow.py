...
capture = cv.CaptureFromFile('C:/test/realdata_baofeng.mp4')
...
tempwriter=cv.CreateVideoWriter("C:/test/pythonfiles/temp.avi", int(codec), int(fps), (width,height), 1)
...
#         cv.WriteFrame(writer, frame)
    #-----------
    if shot < max_frame:
        cv.WriteFrame(tempwriter, frame)
        ...
        del tempwriter
        if weight>myweight:
            tempcapture = cv.CaptureFromFile('C:/test/pythonfiles/temp.avi')
            for f in xrange(75):
                tempframe = cv.QueryFrame(tempcapture)
                cv.WriteFrame(writer, tempframe)
        weight=0
        tempwriter=cv.CreateVideoWriter("C:/test/pythonfiles/temp.avi", int(codec), int(fps), (width,height), 1)
    
    cv.ShowImage("The Video",  output)
    cv.WaitKey(wait)
mat = np.array(weightarr)
np.savetxt("result222.txt", mat);   